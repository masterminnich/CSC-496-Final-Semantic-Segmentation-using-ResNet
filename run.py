...
try:
	...
	for i in range(n_filters):
		...
		for j in range(3):
			...
			pyplot.imsave('layers_j'+str(j)+"_i"+str(i)+'.png', f[:, :, j], cmap='gray')
			ix += 1
	# show the figure
	pyplot.show()
	pyplot.imsave("work.png")
except Exception as e:
	print("error2 "+ str(e))
...
try:
	def plot_filters(layer, x, y):
		...
		for j in range(len(filters)):
			ax = fig.add_subplots(y,x,j+1)
			ax.matshow(filters[j][0], camp = matplotlib.cm.binary)
			plt.xticks(np.array([]))
			plt.yticks(np.array([]))
		plt.tight_layout
		plt.savefig("layer_.png")
		return plt
except:
	print("error")
